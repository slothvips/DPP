...
> 状态：阶段 A-E 已实现；编辑、失败继续和 TestRun 字段合并已纳入扩展实现
>
...
不做独立 Monaco 编辑器，使用结构化表单编辑标题、描述、目标 URL、前置条件、测试数据和步骤。
...
验收：测试步骤按 A -> B -> C 逐个执行；普通模式按现有确认策略工作；YOLO 模式可连续执行；每步结果都能在其他客户端同步后看到。
...
- 独立测试用例编辑器 HTML 入口。
- 自动开启 YOLO。
- 测试失败后自动继续已支持；仍不做失败后自动重试同一个网页操作。
- 复杂版本历史表；先保存执行快照。
