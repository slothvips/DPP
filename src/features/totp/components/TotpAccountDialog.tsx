...
export function TotpAccountDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[86vh] overflow-y-auto border-border/60 bg-background/96 sm:max-w-[460px]">
        <DialogHeader>
          <DialogTitle>{isEdit ? '编辑账户' : '导入账户'}</DialogTitle>
          <DialogDescription>
            {isEdit
              ? '仅可修改名称、发行方与账号。密钥及算法参数导入后不可更改。'
              : '粘贴 otpauth:// 链接或密钥，识别后即可导入。'}
          </DialogDescription>
        </DialogHeader>
          ...
          {!isEdit && (
            <div className="grid gap-2">
              ...
              <Textarea
                ...
                className={cn(
                  'min-h-[120px] resize-y rounded-xl font-mono text-xs leading-5',
