      ...
      try {
        ...
        if (id) {
          ...
          if (!res.success || !res.events) {
            if (!mounted) return;
            throw new Error(res.error || 'Failed to load cached recording');
          }
          events = res.events as RRWebEvent[];
          if (mounted) setRecordingTitle(res.title || 'Remote Recording');
        } else if (url) {
          const res = await fetch(url);
          if (!res.ok) throw new Error('Failed to load recording file');
          events = (await res.json()) as RRWebEvent[];
          if (mounted) setRecordingTitle('External Recording');
        } else {
        ...
        if (events.length === 0) {
          throw new Error('Recording is empty (0 events)');
        }
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground font-sans">
      ...
      <main className="flex-1 flex flex-col justify-center items-center overflow-auto bg-background/50 p-4">
        {loading && <div className="text-muted-foreground text-lg">Loading recording...</div>}
        ...
        {!loading && !error && !hasPlayer && (
          <div className="text-muted-foreground border-2 border-dashed border-border rounded-lg p-12 text-center">
            ...
            <p className="text-sm mt-2">Open a recording from the extension popup.</p>
          </div>
        )}
